...
/******** OWNER NUMBER**********/
const ownerNumber = ["5521979664189@s.whatsapp.net","5521979664189@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oii @${num.split('@')[0]}\Bem vindo carai *${mdata.subject}* Olha as regras hein 🐊`
				...
				teks = `Ja vai tarde ,fdp @${num.split('@')[0]} Nunk serviu pra nada mesmo 🙄'
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `agora e um membro comum 🐊Removi msm :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`agora e um membro comum 🐊Removi msm @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
